# Poobiverse Classic — AMP template
...
In AMP: Configuration → Instance Deployment → Configuration Repositories → Add that source → Fetch. Refresh the browser, then create a separate Poobiverse Classic instance. Do not submit this repository to CubeCoders' community template repo, and do not change the Scratch instance.
...
| Allowed Origins | `AllowedOrigins` | `POOBIVERSE_ALLOWED_ORIGINS` | `https://old.pipenpoob.com` |
